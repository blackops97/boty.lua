local function check_member_super(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = 'تـم تفعيل البوت في المجموعه🔹'..msg.to.title
...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
      local text = '🔹تم تعطيل البوت في المجموعه'..msg.to.title
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local function promote_admin(receiver, member_username, user_id)
  ...
  if data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_username..'انه اداري بلفعل ❣💚 ')
  end
...
local function promote2(receiver, member_username, user_id)
  ...
  if data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_username..'انه اداري ☺️❤️ بلفعل 👌📲 ')
  end
  data[group]['moderators'][tostring(user_id)] = member_tag_username
  save_data(_config.moderation.data, data)
  send_large_msg(receiver, member_username..'العضو 💋 تم رفعه الى اداري تدلل 😚 اسفنجة قلبي ❤️😉 ')
end
...
local function DevPointTeam(msg, matches)
	if msg.to.type == 'chat' then
		if matches[1] == 'تحويل المجموعة الى خارقة' then
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'ازالة البوت' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
		...
		if matches[1] == 'وضع صورة للمجموعة' and is_momod(msg) then
			...
			return 'الرجا ارسال الصورة المراد وضعها ❣✅'
		end
		...
		if matches[1] == 'امنع' and is_owner(msg) then
			local chat_id = msg.to.id
...
return 'تم فتح ارسال الفديو 🎥🔐\n👮 الامر بواسطة :️ @'..msg.from.username 
		...
		if matches[1] == "اسكت" or matches[1] == "unsilent" and is_momod(msg) then
			local chat_id = msg.to.id
			local hash = "mute_user"..chat_id
			local user_id = ""
			if type(msg.reply_id) ~= "nil" then
